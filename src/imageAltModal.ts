	...
	plugin.registerEvent(
		plugin.app.workspace.on("file-menu", (menu, file: TFile) => {
			...
			if (
				editor &&
				selectedText &&
				selectedText.startsWith("![[", 0) &&
				selectedText.endsWith("]]", selectedText.length)
			) {
				menu.addItem((item) => {
...
class ImageAltModal extends Modal {
	...
	onOpen() {
		...
		submitButton.addEventListener("click", async () => {
			...
			const newFilePath = `michalkukla.pl/blog/${newFileName}`;

			if (this.moveFile) {
				const imageFile = this.app.vault.getAbstractFileByPath(
					this.imageName
				) as TFile;
				if (imageFile) {
			...
			const markdownPath = this.moveFile
				? `./${newFileName}`
				: `./${this.imageName}`;
			...
			new Notice(
				this.moveFile
					? "Image updated, moved, and markdown updated successfully!"
					: "Markdown updated successfully!"
			);
			this.close();
		});
	}
